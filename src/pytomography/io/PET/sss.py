...
from .shared import sinogram_coordinates, sinogram_to_spatial, listmode_to_sinogram, listmodeTOF_to_sinogramTOF
...
def get_sample_scatter_points(
    ...
    """
    mgrid = torch.meshgrid(*[torch.arange(0,s,stepsize) for s in attenuation_map.shape[1:]])
    coords = torch.vstack([m.flatten() for m in mgrid])
    idx_above_cutoff = (attenuation_map[0,::stepsize,::stepsize,::stepsize].permute((2,1,0)).cpu().numpy().T>attenuation_cutoff).flatten()
...
def compute_sss_sparse_sinogram(
    ...
    for scatter_point in range(coords.shape[1]):
        ...
        mu_value = attenuation_image[0][tuple(coords[:,scatter_point].tolist())]
        ...
        transmission_integrals = parallelproj.joseph3d_fwd(
            scatter_point_position.unsqueeze(0).expand(scanner_LUT.shape[0], -1),
            scanner_LUT.to(pytomography.device),
            attenuation_image.to(pytomography.dtype).to(pytomography.device)[0],
            object_origin,
            object_meta.dr,
        )
        ...
        counts += 1
    scatter_sinogram_sparse = gate.listmode_to_sinogram(detector_ids_scatter, proj_meta.info, weights=(probability/counts).cpu())
...
def compute_sss_sparse_sinogram_TOF(
    ...
    for scatter_point in range(coords.shape[1]):
        ...
        offset_SB = -offset_SA
        prob_SA = tof_efficiency(offset_SA, bin_centers_distance_along_LOR[idxA], tof_meta) # first dim TOFbin
        prob_SB = tof_efficiency(offset_SB, bin_centers_distance_along_LOR[idxB], tof_meta) # first dim TOFbin
    ...
    scatter_sinogram_sparse = gate.listmodeTOF_to_sinogramTOF(detector_ids_scatter_with_TOF, proj_meta.info, tof_meta, weights=(probability/counts).cpu())
    return scatter_sinogram_sparse
...
def scale_estimated_scatter(
    ...
    # Scatter
    BP_scatter_mask = system_matrix.backward(~proj_data_mask*proj_scatter.to(system_matrix.output_device)) / norm_BP
    # Random
    if sinogram_random is not None:
        ...
        BP_random_mask = 0
    # Total
    BP_total_mask = system_matrix.backward(~proj_data_mask*proj_data.to(system_matrix.output_device)) / norm_BP
...
def get_sss_scatter_estimate(
    ...
    num_dense_tof_bins: int = 25) -> torch.Tensor:
    ...
    if tof_meta is None:
        ...
        # Get sparse sinogram
        scatter_sinogram_sparse_unscaled = compute_sss_sparse_sinogram_TOF(object_meta, proj_meta, pet_image, attenuation_image, tof_meta, image_stepsize, attenuation_cutoff, sinogram_interring_stepsize, sinogram_intraring_stepsize, num_dense_tof_bins)
        scatter_sinogram_unscaled = []
        # Interpolate sparse sinogram (loop over TOF bins)
        for i in range(scatter_sinogram_sparse_unscaled.shape[-1]):
            scatter_sinogram_unscaled.append(interpolate_sparse_sinogram(scatter_sinogram_sparse_unscaled[:,:,:,i], proj_meta, *get_sample_detector_ids(proj_meta, sinogram_interring_stepsize, sinogram_intraring_stepsize)[:2]))
        scatter_sinogram_unscaled = torch.stack(scatter_sinogram_unscaled, dim=-1)
    del(scatter_sinogram_sparse_unscaled) # save memory for nect step
    ...
    if listmode:
        ...
        if tof_meta is None:
            ...
            proj_data = listmodeTOF_to_sinogramTOF(proj_meta.detector_ids.cpu(), proj_meta.info, tof_meta)
    # Scale sinogram
